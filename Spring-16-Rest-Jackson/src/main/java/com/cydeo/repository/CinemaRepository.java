...
public interface CinemaRepository extends JpaRepository<Cinema, Long> {
    ...
    @Query(value = "select c from Cinema c where c.location.country=:country",nativeQuery = true)
    List<Cinema> findAllByLocationCountry(String country);
    //Write a native query to read all cinemas by name or sponsored name contains a specific pattern
    @Query(value = "select c from Cinema c where c.name or c.sponsoredName=:pattern",nativeQuery = true)
    List<Cinema> findByNameOrSponsorNameContains(String pattern);
    //Write a native query to sort all cinemas by name
    @Query(value = "select c from Cinema c order by c.name",nativeQuery = true)
